...
class UnsupervisedMaskIoUMetric(torchmetrics.Metric):
    ...
    def update(
        ...
        for pred, target in zip(predictions, targets):
            ...
            iou_per_class = unsupervised_mask_prec_recall(
                pred, target, matching=self.matching, reduction="none", metric=self.metric
            )
...
class BestOverlapPixelRecallMetric(UnsupervisedMaskIoUMetric):
    def __init__(self, **kwargs):
        super().__init__(matching="best_overlap", metric="recall", **kwargs)

class BestOverlapPixelPrecisionMetric(UnsupervisedMaskIoUMetric):
    def __init__(self, **kwargs):
        super().__init__(matching="best_overlap", metric="precision", **kwargs)
...
def unsupervised_mask_iou(
    ...
    if matching == "hungarian":
        ...
        pred_idxs = torch.argmax(pairwise_iou, dim=0)[non_empty_gt.cpu()]
        true_idxs = torch.arange(pairwise_iou.shape[1])[non_empty_gt.cpu()]
    else:
...
def unsupervised_mask_prec_recall(
    pred_mask: torch.Tensor,
    true_mask: torch.Tensor,
    metric: str,
    matching: str = "hungarian",
    ...
    assert pred_mask.ndim == 2
    assert true_mask.ndim == 2
    assert metric in ("precision", "recall", "iou")
    n_gt_classes = len(true_mask)
    pred_mask = pred_mask.unsqueeze(1).to(torch.bool)
    true_mask = true_mask.unsqueeze(0).to(torch.bool)
    ...
    pred_size = torch.sum(pred_mask, dim=-1).to(torch.float64)
    true_size = torch.sum(true_mask, dim=-1).to(torch.float64)

    union = torch.sum(pred_mask | true_mask, dim=-1).to(torch.float64)
    pairwise_iou = intersection / union

    recall = intersection / true_size
    precision = intersection / pred_size
    ...
    matched_iou = pairwise_iou[pred_idxs, true_idxs]
    matched_recall = recall[pred_idxs, true_idxs]
    matched_precision = precision[pred_idxs, true_idxs]

    iou = torch.zeros(n_gt_classes, dtype=torch.float64, device=pairwise_iou.device)
    iou[true_idxs] = matched_iou

    recall = torch.zeros(n_gt_classes, dtype=torch.float64, device=pairwise_iou.device)
    recall[true_idxs] = matched_recall
    precision = torch.zeros(n_gt_classes, dtype=torch.float64, device=pairwise_iou.device)
    precision[true_idxs] = matched_precision
