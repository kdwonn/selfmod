...
from ocl.slot_dict.block import BlockLinear
...
def build_mlp(
    ...
    get_linear = nn.Linear if num_blocks == 1 \
        else lambda d_in, d_out: BlockLinear(d_in, d_out, num_blocks)
    layers = []
    ...
    for n_features in features:
        layers.append(get_linear(current_dim, n_features))
        nn.init.zeros_(layers[-1].bias)
        layers.append(get_activation_fn(activation_fn))
        current_dim = n_features

    layers.append(get_linear(current_dim, output_dim))
    nn.init.zeros_(layers[-1].bias)
...
def build_two_layer_mlp(
    input_dim, output_dim, hidden_dim, initial_layer_norm: bool = False, residual: bool = False, num_blocks=1
):
    ...
    """
    return build_mlp(
        input_dim, output_dim, [hidden_dim], initial_layer_norm=initial_layer_norm, residual=residual, num_blocks=num_blocks
    )
