...
from ocl.neural_networks.convenience import get_activation_fn, build_two_layer_mlp
...
@dataclasses.dataclass
class BlobReconstructionOutput:
    reconstruction: TensorType["batch_size", "n_patches", "n_patch_features"]  # noqa: F821
    masks: TensorType["batch_size", "n_objects", "n_patches"]  # noqa: F821
    masks_as_image: Optional[
        TensorType["batch_size", "n_objects", "height", "width"]  # noqa: F821
    ] = None
    blobs_as_image: Optional[
        TensorType["batch_size", "n_objects", "height", "width"]  # noqa: F821
    ] = None
    target: Optional[TensorType["batch_size", "n_patches", "n_patch_features"]] = None  # noqa: F821
    cov_xx: Optional[torch.tensor]  = None
    cov_yy: Optional[torch.tensor]  = None
    shift: Optional[torch.tensor]  = None
    ab_norm: Optional[torch.tensor]  = None
...
class InvTransScalePatchDecoder(nn.Module):
    """Decoder that takes object representations and reconstructs patches.
    ...
    def __init__(
        ...
        self.abs_grid = get_abs_grid(self.num_patches)
        self.grid_proj = nn.Linear(2, object_dim)
    ...
    def forward(
        ...
        with torch.cuda.amp.autocast(enabled=not self.off_amp):
            pos, scale = position_features[..., :2], position_features[..., -2:]
            abs_grid = self.abs_grid.to(device=pos.device, dtype=pos.dtype)
            abs_grid = rearrange(abs_grid, 'n d -> 1 1 n d')

            rel_grid = abs_grid - pos.unsqueeze(2)
            rel_grid = rel_grid / (self.scale_factor * scale.unsqueeze(2))

            pos_embed = self.grid_proj(rel_grid)

        # Simple learned additive embedding as in ViT
        object_features = object_features.unflatten(0, initial_shape)
        object_features = object_features + pos_embed
...
class BlobDecoder(nn.Module):
    ...
    def __init__(
        ...
        upsample_target: Optional[float] = None,
        resize_mode: str = "bilinear",
        feature_jitter_angle: float = 0,
        feature_jitter_shift: float = 0,
        feature_jitter_xy: float = 0,
        ab_norm: Optional[float] = None,
        use_shift: bool = True,
        shift_bound: float = 1,
        ab_norm_warmup: int = 0,
        use_null: bool = False,
        max_ab_norm: float = 0.33,
    ):
        super().__init__()
        self.output_dim = output_dim
        self.num_patches = num_patches
        ...
        if use_null:
            self.pos_embed = nn.Parameter(torch.randn(1, decoder_input_dim) * 0.02)
        else:
            self.pos_embed = nn.Parameter(torch.randn(1, num_patches, decoder_input_dim) * 0.02)

        self.blob_mlp = build_two_layer_mlp(position_dim, 2+4+1, position_dim // 2)
        self.feature_jitter_angle = feature_jitter_angle
        self.feature_jitter_shift = feature_jitter_shift
        self.feature_jitter_xy = feature_jitter_xy
        self.ab_norm = ab_norm
        self.use_shift = use_shift
        self.shift_bound = shift_bound

        self.ab_norm_warmup = ab_norm_warmup
        self.current_iter = 0
        self.max_ab_norm = max_ab_norm
        assert self.ab_norm <= max_ab_norm

    def get_ab_norm_warmup(self):
        if self.current_iter < self.ab_norm_warmup:
            return (self.max_ab_norm - self.ab_norm) * (self.ab_norm_warmup - self.current_iter) / self.ab_norm_warmup + self.ab_norm
        else:
            return self.ab_norm
    ...
    def compute_delta_map(self, xs, ys):
        # * compute relative delta
        feature_coords = torch.stack((xs, ys), -1).mul(self.grid_size)  # [n, m, 2]
        grid_coords = torch.stack((
            torch.arange(self.grid_size).repeat(self.grid_size), 
            torch.arange(self.grid_size).repeat_interleave(self.grid_size)
        )).to(xs.device)  # [2, size*size]
        delta = (grid_coords[None, None] - feature_coords[..., None]).div(self.grid_size)  # [n, m, 2, size*size]
        return delta
    
    def compute_covariance_matrix(self, covs):
        #  a, b: diagonal covariance
        a, b = covs[..., :2].sigmoid().unbind(-1)
        # * ab_norm * (a * b)^-0.5
        # * Diagonal convs are sclaed with this parameter, which eventually make diagonals to be the reciprocals
        # ? Can we just predict single parameter a and make diagonals to be a and 1/a, as described in the paper? 
        ab_norm = 1
        if self.ab_norm is not None:
            ab_norm = self.get_ab_norm_warmup()
            ab_norm = ab_norm * (a * b).rsqrt()

        # * Normalized orientation vector: [cos, sin]
        # * Rotation matrix: [[cos, -sin], [sin, cos]]
        basis_i = covs[..., 2:]
        basis_i = F.normalize(basis_i, p=2, dim=-1)
        basis_i[..., 1] = -basis_i[..., 1]
        if self.feature_jitter_angle and self.training:
            with torch.no_grad():
                theta = basis_i[..., 0].arccos()
                theta += torch.empty_like(theta).uniform_(
                    -self.feature_jitter_angle, self.feature_jitter_angle
                )
                basis_i_jitter = (self.rotation_matrix(theta)[..., 0] - basis_i).detach()
            basis_i = basis_i + basis_i_jitter
        basis_j = torch.stack((-basis_i[..., 1], basis_i[..., 0]), -1)
        R = torch.stack((basis_i, basis_j), -1).to(device=covs.device, dtype=covs.dtype)

        # * by multiplyinh ab_norm, diagonals are reciprocals scaled with self.ab_norm
        covs = torch.zeros_like(R).to(device=covs.device, dtype=covs.dtype)
        covs[..., 0, 0] = a * ab_norm
        covs[..., -1, -1] = b * ab_norm
        covs = torch.einsum('...ij,...jk,...lk->...il', R, covs, R)
        # * epsilon for diagonal covariance, for computational stability?
        covs = covs + torch.eye(2)[None, None].to(device=covs.device, dtype=covs.dtype) * 1e-5
        return covs, ab_norm
    
    def compute_mahalanobis(self, covs, delta):
        sq_mahalanobis = (
            delta * torch.linalg.solve(covs.to(torch.float32), delta.to(torch.float32))).sum(2)
        sq_mahalanobis = rearrange(
            sq_mahalanobis, 'n m (s1 s2) -> n m s1 s2', s1=self.grid_size).to(device=covs.device, dtype=covs.dtype)
        return sq_mahalanobis
    ...
    def splat(
        ...
        Args:
            ...
            covs: [N, M, 2, 4] xy covariance matrices for each feature
            shift: [N, M+1] distributions of per feature (and bg) weights
        ...
        """
        if self.feature_jitter_xy and self.training:
        ...
        covs, ab_norm = self.compute_covariance_matrix(covs)
        delta = self.compute_delta_map(xs, ys)
        sq_mahalanobis = self.compute_mahalanobis(covs, delta)

        if self.use_shift:
            shift = shift[:, :, None, None].tanh() * self.shift_bound
            if self.feature_jitter_shift and self.training:
                shift = shift + torch.empty_like(shift).uniform_(-self.feature_jitter_shift, self.feature_jitter_shift)
        else:
            shift = torch.zeros_like(shift[:, :, None, None])
        scores = nn.functional.sigmoid(-sq_mahalanobis + shift)
    ...
    def forward(
        ...
        assert object_features.dim() >= 3  # Image or video data.
        assert position_features.dim() >= 3  # Image or video data.

        blob_params = self.blob_mlp(position_features)
        splat_map, blob_stats = self.splat(
            xs=blob_params[..., 0].sigmoid(),
            ys=blob_params[..., 1].sigmoid(),
            covs=blob_params[..., 2:-1],
            shift=blob_params[..., -1]
        )
        ...
        object_features = rearrange(object_features, 'b n d -> b n 1 d')
        object_splat = object_features * rearrange(splat_map, 'b n h w -> b n (h w) 1')
        if self.use_null:
            null_splat = self.pos_embed * (1 - rearrange(splat_map, 'b n h w -> b n (h w) 1'))
            object_splat = null_splat + object_splat
        else:
            object_splat = self.pos_embed + object_splat

        output = self.decoder(object_splat)
        # output = output.unflatten(0, initial_shape)
        ...
        return BlobReconstructionOutput(
            reconstruction=reconstruction,
            masks=alpha.squeeze(-1),
            masks_as_image=masks_as_image,
            blobs_as_image=splat_map,
            target=target if target is not None else None,
            **blob_stats
        )
...
class AutoregressivePatchDecoder(nn.Module):
    ...
    def __init__(
        ...
        max_neg_value = -torch.finfo(self.decoder.layers[0].linear1.weight.dtype).max
        mask = torch.triu(torch.full((num_patches, num_patches), max_neg_value), diagonal=1)
        self.register_buffer("mask", mask)

        self.inp_masking_prob = inp_masking_prob
        if self.inp_masking_prob > 0:
            self.inp_mask = nn.Parameter(torch.rand(decoder_dim))
        else:
            self.inp_mask = None
    ...
    def forward(
        ...
        # NOTE: add detach also in this case?
        targets = target.detach().flatten(0, -3)
        ...
        if self.inp_mask is not None and self.training:
            # repace input patch tokens with the self.inp_mask with probability inp_masking_prob
            mask = torch.rand_like(inputs[:, :, 0]) < self.inp_masking_prob
            inputs[mask] = self.inp_mask
        ...
        if empty_objects is not None:
            outputs = self.decoder(
                inputs,
                object_features,
                self.mask if input_masks is None else input_masks,
