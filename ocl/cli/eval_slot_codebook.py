...
def evaluate(config: EvaluationClusteringConfig):
    # Create output dir to save codebooks knn images
    target_dir = hydra.utils.to_absolute_path(config.train_config_path).split('/')[:-1]
    target_dir = '/'.join(target_dir)
    output_dir = target_dir + '/codebook'
    ...
    codebook_size = train_config.models.feedback_model.pool.codebook_size
    ...
    for idx in range(codebook_size):
        ...
        img_output_dir = output_dir + f"/{idx:03}"
        os.makedirs(img_output_dir,  exist_ok=True)
        for img_idx, img in enumerate(masked_image_per_codebook[idx][:20]):
            torchvision.utils.save_image(img, img_output_dir + f"/{img_idx:02}.jpg", normalize=True, range=(0, 1))
