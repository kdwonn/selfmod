...
class ScalarWrapperLoss(nn.Module):
    """ wrapper for the loss scalar that is already computed externally
    """
    def __init__(self, weight: float = 1.0):
        super().__init__()
        self.weight = weight

    def forward(self, loss: torch.Tensor) -> float:
        return self.weight * loss


class AttentionMatchingLoss(nn.Module):
    def __init__(self, weight: float = 1.0, measure='ce'):
        super().__init__()
        self.weight = weight
    ...
    def forward(self, input, target):
        ...
        # pattern = 'b k n -> (b k) n'
        # input = rearrange(input, pattern)
        # target = rearrange(target, pattern)

        target = target.detach()

        if self.measure == 'ce':
            #  CE loss is computed across second dimension
            loss = self.loss_fn(input, target)
        elif self.measure == 'kl':
            # KL loss is computed pointwise & input should be log-probabilities
            # NOTE Checked that with current implementation, the results of CE loss = KL loss + entropy
            loss = self.loss_fn(
                rearrange(input, 'b k n -> (b n) k').log(),
                rearrange(target, 'b k n -> (b n) k'),
            )
