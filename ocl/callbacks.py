...
class GumbelTempAnnealing(Callback):
    def __init__(self, initial_temp, starting_iter, iter_to_anneal, min_temp=0.2):
        #  anout 100k to reach min temp
        self.initial_temp = initial_temp
        self.anneal_rate = math.log(min_temp/initial_temp) / float(iter_to_anneal)
        self.min_temp = min_temp
        self.starting_iter = starting_iter

    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        global_step = trainer.global_step - self.starting_iter
        if global_step < 0:
            return
        new_temp = max(self.initial_temp * math.exp(self.anneal_rate*global_step), self.min_temp)
        pl_module.models.perceptual_grouping.slot_attention.gumbel_temperature = new_temp


class FeedbackProbAnnealing(Callback):
    def __init__(self, initial_prob, iter_to_anneal, starting_iter) -> None:
        super().__init__()
        self.initial_prob = initial_prob
        self.iter_to_anneal = iter_to_anneal
        self.starting_iter = starting_iter

    def on_train_batch_start(self, trainer: Trainer, pl_module: LightningModule, batch: Any, batch_idx: int) -> None:
        # 1- cosine annealing of feedback prob.
        # starts from initial_prob and goes to 1
        global_step = trainer.global_step - self.starting_iter
        if global_step < 0:
            pl_module.models.feedback_model.feedback_prob = self.initial_prob
            return
        new_prob = self.initial_prob + 0.5 * (1 - self.initial_prob) * (1 - math.cos(math.pi * global_step / self.iter_to_anneal))
        pl_module.models.feedback_model.feedback_prob = new_prob


class FeedbackProbOffOn(Callback):
    def __init__(self, starting_iter) -> None:
        super().__init__()
        self.starting_iter = starting_iter

    def on_train_batch_start(self, trainer: Trainer, pl_module: LightningModule, batch: Any, batch_idx: int) -> None:
        #  first 0, after starting_iter 1
        global_step = trainer.global_step - self.starting_iter
        if global_step < 0:
            pl_module.models.feedback_model.feedback_prob = 0
            return
        pl_module.models.feedback_model.feedback_prob = 1
