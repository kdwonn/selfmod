...
from ocl.feature_extractors.utils import (
    ...
    ImageFeatureExtractorPrompt,
    cnn_compute_positions_and_flatten,
    transformer_compute_positions,
)
from ocl.feature_extractors.vpt import VPT_ViT, VPT_ViT_wo_fb, VPT_ViT_code
...
class _VitFeatureHook:
    ...
    def __init__(self, feature_type: _VitFeatureType, block: int, drop_cls_token: bool = True, num_out_tokens = None):
    ...
    @staticmethod
    def create_hook_from_feature_level(
        feature_level: Union[int, str],
        num_out_tokens = None,
    ):
        ...
        return _VitFeatureHook(feature_type, block, num_out_tokens=num_out_tokens)
    ...
    def __call__(self, module, inp, outp):
        if self.feature_type == _VitFeatureType.BLOCK:
            ...
            if self.drop_cls_token:
                ...
                if self.num_out_tokens is not None:
                    features = features[:, 1:1+self.num_out_tokens]
                else:
                    features = features[:, 1:]
...
class TimmFeatureExtractorPrompt(ImageFeatureExtractorPrompt):
    """Feature extractor implementation for timm models, with prompt
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.is_vit:
            if wo_fb:
                ...
                model = build_model_with_cfg(
                    ...
                    pretrained=pretrained,
                    pretrained_strict=False,
                    num_per_slot=num_per_slot,
                    use_deep=use_deep,
                    use_inter_share=use_inter_share,
                    code_dim=code_dim,
                    use_mlp=use_mlp,
                    use_ln=use_ln,
                    use_running_prompt=use_running_prompt,
                    num_slot=num_slot,
                    **kwargs
                )
            else:
            ...
            assert len(model.patch_embed.img_size) == 2, "Only 2D images supported"
            assert len(model.patch_embed.patch_size) == 2, "Only 2D images supported"

            self.num_patch_tokens = (model.patch_embed.img_size[0] // model.patch_embed.patch_size[0]) ** 2
            ...
            if len(self.feature_levels) > 0 or len(self.aux_features) > 0:
                self._feature_hooks = [
                    _VitFeatureHook.create_hook_from_feature_level(
                        level, 
                        num_out_tokens=None if self.return_prompt else self.num_patch_tokens
                    ).register_with(model)
                    for level in itertools.chain(self.feature_levels, self.aux_features)
                ]
                if len(self.feature_levels) > 0:
            ...
            raise NotImplementedError("Prompt only supported for ViT models")
        ...
        if freeze:
            self.model.freeze_except_prompt()
            # BatchNorm layers update their statistics in train mode. This is probably not desired
            # when the model is supposed to be frozen.
            contains_bn = any(
                isinstance(m, (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d))
    ...
    def forward_images(self, images: torch.Tensor, code: torch.Tensor, indices: torch.Tensor):
        if self.run_in_eval_mode and self.training:
            self.eval()
        ...
        if self.is_vit:
            no_grad = indices is None and code is None and self.freeze and self.n_blocks_to_unfreeze == 0 and not self.wo_fb
            if no_grad:
                # Speed things up a bit by not requiring grad computation.
                with torch.no_grad():
                    features = self.model.forward_features(images, code, indices)
            else:
                features = self.model.forward_features(images, code, indices)
            ...
            if len(self.feature_levels) == 0:
                ...
                if self.return_prompt:
                    prompt = [f[:, self.num_patch_tokens:] for f in features]
                    features = [f[:, :self.num_patch_tokens] for f in features]
        ...
        return features, positions, aux_features, prompt
