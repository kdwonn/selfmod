...
class ImageFeatureExtractorPrompt(FeatureExtractor):
    """
    """

    @abc.abstractmethod
    def forward_images(
        self, images: ocl.typing.ImageData, indices: torch.Tensor
    ) -> Union[
        Tuple[ocl.typing.ImageFeatures, ocl.typing.Positions],
        Tuple[ocl.typing.ImageFeatures, ocl.typing.Positions, Dict],
    ]:
    ...
    def forward(
        ...
        result = self.forward_images(images, code=code, indices=indices)
        ...
        if isinstance(result, (Tuple, List)):
            if len(result) == 2:
                ...
                features, positions, aux_features, prompt = result
            else:
                raise RuntimeError("Expected either 2 or 3 element tuple from `forward_images`.")
        else:
            # Assume output is simply a tensor without positional information.
        ...
        return ocl.typing.FeatureExtractorOutput(features, positions, aux_features, prompt)
