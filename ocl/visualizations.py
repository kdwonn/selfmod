...
from ocl.utils import resizing
...
class ResizedMask(Mask):
    def __init__(self, target_size: int, patch_mode: bool, **kwargs):
        super().__init__(**kwargs)
        self.target_size = target_size
        self.resizer = resizing.Resize(target_size, patch_mode=patch_mode)

    def __call__(self, mask: torch.Tensor) -> visualization_types.Image:
        mask = self.resizer(mask)
        return super().__call__(mask)
    

class IndexCaptionedImage(VisualizationMethod):
    def __init__(self, image_vis: Union[Mask, Image], n_instances: int=4) -> None:
        self.image_vis = image_vis
        self.n_instances = n_instances

    def __call__(self, image: torch.Tensor, indices: torch.Tensor) -> visualization_types.Image:
        image = self.image_vis(image)
        indices = indices[:self.n_instances].cpu().contiguous()
        if isinstance(image, visualization_types.Image):
            caption = ""
            for i in range(indices.shape[0]):
                idx_string = str(indices[i]).replace(",\n      ", "")[7:-1]
                caption += f"L{i}: {idx_string}\n"
            image.caption = [caption]
        elif isinstance(image, visualization_types.Images):
            caption = []
            for i in range(indices.shape[0]):
                idx_string = str(indices[i]).replace(",\n      ", "")[7:-1]
                caption += [f"L{i}: {idx_string}\n"]
...
class MaskedImage(VisualizationMethod):
    def __init__(
        self,
        n_images: int = 4,
        n_objects: int = 7,
        ...
        self.image_vis = Image(n_instances=n_images, n_row=n_objects, as_grid=False, denormalization=denormalization)

    def __call__(
        self, mask: torch.Tensor, image: torch.Tensor
    ) -> Union[visualization_types.Image, visualization_types.Video]:
        """Visualize mask.
        ...
        Returns:
            Visualized mask.
        """
        masks = mask[: self.n_images].cpu().contiguous()
        image_shape = masks.shape[-2:]
        n_objects = masks.shape[-3]

        image = self.image_vis(image[: self.n_images]).img_tensor

        assert masks.dim() == 4
        # Handling image data.
        # bs x objects x H x W
        # Monochrome image with single channel.
        masks = rearrange(masks, 'b o h w -> b o () h w')
        image = rearrange(image, 'b c h w -> b () c h w')
        masked_image = rearrange(masks * image, 'b o c h w -> (b o) c h w')
        # Draw masks inverted as they are easier to print.
        return visualization_types.Image(make_grid(masked_image, nrow=n_objects))


class IndexHistogram(VisualizationMethod):
    def __init__(self, pool_size: int) -> None:
        super().__init__()
        self.pool_size = pool_size

    def __call__(self, indices: torch.tensor) -> visualization_types.Histogram:
        np_indices = np.histogram(indices.cpu().data.numpy())
        return visualization_types.Histogram(np_indices)
