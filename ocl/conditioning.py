...
class RandomConditioning(nn.Module):
    ...
    def forward(self, batch_size: int, num_slots=None) -> ocl.typing.ConditioningOutput:
...
class RandomConditioningTransScale(RandomConditioning):
    def __init__(self, object_dim: int, n_slots: int, learn_mean: bool = True, learn_std: bool = True, mean_init: Callable[[torch.Tensor], None] = torch.nn.init.xavier_uniform_, logsigma_init: Callable[[torch.Tensor], None] = nn.init.xavier_uniform_):
        super().__init__(object_dim, n_slots, learn_mean, learn_std, mean_init, logsigma_init)

        self.scale_sampling = torch.distributions.normal.Normal(0.1, 0.1)
        self.pos_sampling = torch.distributions.uniform.Uniform(-1, 1)
    ...
    def forward(self, batch_size: int) -> ocl.typing.ConditioningOutput:
        ...
        obj_slots = mu + sigma * torch.randn_like(mu)

        pose_shape = batch_size, self.n_slots, 2
        pos = self.pos_sampling.sample(pose_shape).to(
            device=obj_slots.device, dtype=obj_slots.dtype)
        scale = self.scale_sampling.sample(pose_shape).to(
            device=obj_slots.device, dtype=obj_slots.dtype)

        return torch.cat([obj_slots, pos, scale], dim=-1)
