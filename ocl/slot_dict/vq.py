...
def gumbel_sample(
    ...
    if reinmax:
        ...
        pi1 = ((log(pi1) - logits).detach() + logits).softmax(dim = 1)
        pi2 = 2 * pi1 - 0.5 * pi0
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        ema_update = True,
        learnable_codebook = False,
        in_place_codebook_optimizer = False, # Optimizer used to update the codebook embedding if using learnable_codebook
        affine_param = False,
        affine_param_batch_decay = 0.99,
        affine_param_codebook_decay = 0.9,
        ...
        self.in_place_codebook_optimizer = torch.optim.SGD(self._codebook.parameters(), lr=1-decay) if in_place_codebook_optimizer else None
    ...
    def forward(
        ...
        if self.training:
            ...
            # straight through
            # * VQ straight through impl.
            # * straight trhough option only controls the gradients for logits
